...
/* ── SVG Social Icons (clean, minimal) ── */
const FacebookIcon = () => (
...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav className="mx-auto flex items-center justify-between max-w-[96rem] px-6 py-5 lg:px-12">
        ...
        <div className="hidden lg:flex items-center gap-1">
          {socials.map(({ href, label, Icon }) => (
            <a
              ...
              aria-label={label}
              className="p-2.5 text-white/80 hover:text-gold transition-colors duration-300"
            >
              <Icon />
            </a>
          ))}
        ...
        {/* ── Desktop menu trigger (replaces hamburger on desktop) ── */}
        <button
          onClick={() => setOpen(!open)}
          className="hidden lg:flex items-center gap-2 p-2 text-white/80 hover:text-gold transition-colors duration-300"
          aria-label="Menú"
        >
          <span className="text-xs tracking-[.2em] uppercase font-medium">
            {open ? "Cerrar" : "Menú"}
          </span>
      ...
      <div
        className={`fixed inset-0 bg-navy/98 backdrop-blur-2xl z-40 transition-all duration-500 flex flex-col items-center justify-center ${
      ...
      >
        <nav className="flex flex-col items-center gap-2">
          {links.map(({ label, href }, i) => (
            <a
              ...
              className="text-white/70 hover:text-gold text-3xl lg:text-4xl font-light tracking-wide transition-all duration-300 py-3"
              style={{ transitionDelay: open ? `${i * 60}ms` : "0ms", opacity: open ? 1 : 0, transform: open ? "translateY(0)" : "translateY(16px)" }}
            >
              {label}
            </a>
          ))}
        ...
        <div
          className="flex items-center gap-2 mt-12 transition-all duration-500"
          style={{ transitionDelay: open ? "500ms" : "0ms", opacity: open ? 1 : 0 }}
        >
          {socials.map(({ href, label, Icon }) => (
            <a
              key={label}
              ...
              rel="noopener noreferrer"
              aria-label={label}
              className="p-3 text-white/60 hover:text-gold transition-colors duration-300"
            >
              <Icon />
            </a>
          ))}
