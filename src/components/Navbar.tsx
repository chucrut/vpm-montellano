  ...
  return (
    <>
      <header
        ...
        role="banner"
      >
        <nav className="mx-auto flex items-center justify-between max-w-[90rem] px-5 sm:px-8 py-4 lg:px-12" aria-label="Navegación principal">
          {/* Logo */}
          <a href="#inicio" className="flex items-center gap-3 shrink-0 group z-50" aria-label="VPM Montellano — Ir al inicio">
            <Image
              src="/assets/logo.png"
            ...
            />
            <span className={`font-heading font-semibold tracking-wide text-lg transition-colors duration-500 ${scrolled ? "text-white" : "text-white"}`}>
              VPM
            </span>
          </a>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {links.slice(1).map(({ label, href }) => (
              <a
                key={href}
                href={href}
                className={`px-4 py-2 text-sm tracking-wider transition-colors duration-300 font-medium ${
                  scrolled ? "text-white/80 hover:text-gold" : "text-white/80 hover:text-gold"
                }`}
              >
                {label}
              </a>
          ...
          <div className="flex items-center gap-2 z-50">
            <div className="hidden sm:flex items-center gap-1">
              {socials.map(({ href, label, Icon }) => (
                <a
                  ...
                  aria-label={label}
                  className={`p-3 transition-colors duration-300 min-w-[48px] min-h-[48px] flex items-center justify-center ${
                    scrolled ? "text-white/60 hover:text-gold" : "text-white/70 hover:text-gold"
                  }`}
                >
                  <Icon />
                </a>
            ...
            <button
              ...
              className={`p-3 min-w-[48px] min-h-[48px] flex items-center justify-center transition-colors duration-300 ${
